...
notesApp.service('myService' ,['$interval', function ($interval) {
    ...
    self.addcomment = function(note,inputcomment, usernamet){
        ...
        var newPostRef = hopperRef2.child(idt).set({id:idt, commentname:inputcomment, commentdate:date.toString(),username:usernamet});
         if(note.comment == undefined){
            note.push({comment:"[{id:0, commentname:$scope.inputcomment, commentdate:date1.toString(),username:usernamet}]"});
        }   
        else{
            note.comment.push({id:(note.comment.length+''), commentname:inputcomment, commentdate:date.toString(),username:usernamet});
        } 
    }
     //xoa itemlist
